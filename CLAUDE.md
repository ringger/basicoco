...
- `GPRINT(x,y),"text"[,color]` draws text on the graphics screen using a 4x6 pixel font. Implemented as registry command in `graphics.py` (server emits `{'type': 'gtext', ...}`), rendered client-side by `drawText()` using `GPRINT_FONT` bitmap data in `dual_monitor.js`. That table is the font's only copy: the server reads it (`graphics.gprint_font()`) to record GPRINT pixels for PPOINT
- PPOINT reads the server's `pixel_buffer`, which mirrors everything the client draws (LINE, CIRCLE, DRAW, PSET/PRESET, PAINT, GPRINT, GET/PUT, and the colour state from COLOR/PCLS/PMODE). Server and client must use the same rules (`_record_paint` copies `GraphicsDisplay.paint`, `_record_put` copies `putGraphics`, …). The Node harness compares pixels, and `test_ppoint_matches_the_canvas` (browser) compares PPOINT with the canvas after mixed command sequences — add a case there for any new drawing command
