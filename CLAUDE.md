...
- PPOINT reads the server's `pixel_buffer`, which mirrors what the client draws (LINE, CIRCLE, DRAW, PSET, PAINT, GPRINT; not yet PUT). Server and client must use the same rules: `_record_paint` copies `GraphicsDisplay.paint`, and the Node harness checks each against the other pixel for pixel
