...
class TestProcessLineASTFallback:
    ...
    def test_malformed_for_falls_through(self, basic, helpers):
        ...
        assert errors and errors[0].startswith('SYNTAX ERROR')
        assert helpers.get_text_output(result) == []  # PRINT "X" never ran

    def test_malformed_while_falls_through(self, basic, helpers):
        """Malformed WHILE (no condition) is a syntax error, not a crash"""
    ...
    def test_empty_do_loop_is_stopped_by_iteration_guard(self, basic, helpers):
        """DO: : LOOP is a legal but endless loop; the safety guard stops it"""
        result = basic.process_line('DO: : LOOP')
        errors = helpers.get_error_messages(result)
        assert errors and 'TOO MANY ITERATIONS' in errors[0]
