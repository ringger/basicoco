"""
Test U-turn alignment routines in isolation.

Validates AlignTop (edge alignment), AlignCornerTop (corner alignment),
and AlignMidEdge (middle-layer edge alignment) by placing pieces at known
top-layer positions and verifying they move to the correct target.
"""
...
class TestEdgeAlignment:
    ...
    def test_align_top_edge(self, basic, helpers, start_ep, target_ti, u_moves):
        ...
        out = _run(basic, helpers, [
            # Kick front edge (TC1=7, TC2=4) to top via FF
            f'40 MS$="FF": GOSUB DoMoves',
...
class TestCornerAlignment:
    ...
    @pytest.mark.parametrize("start_cp,target_ci", [
        ...
        (4, 1),  # CP4 -> CI1 (target CP5)
        (4, 2),  # CP4 -> CI2 (target CP6)
        (4, 3),  # CP4 -> CI3 (target CP7)
...
class TestMidEdgeAlignment:
    """Test AlignMidEdge: align top edge so side sticker matches face center."""
