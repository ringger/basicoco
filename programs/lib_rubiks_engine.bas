...
REM   ROT-X TURNS THE SAME WAY AS L (STANDARD x'); ROT-Y FOLLOWS U,
REM   ROT-Z FOLLOWS F
REM   DR: 1=CW   -1=CCW
...
REM   X Y Z WHOLE-CUBE ROTATIONS (AND x y z)
REM   S STANDING MIDDLE SLICE (F DIRECTION)
...
REM
Scramble:
  REM APPLY NM RANDOM FACE TURNS (R U F L D B ONLY: WHOLE-CUBE
  REM ROTATIONS WOULD MOVE THE CENTERS, WHICH THE SOLVER ASSUMES FIXED)
  PRIVATE MV, LT, LD
  LT=0: LD=0
  FOR MV=1 TO NM
    ScramblePick:
    TF=RND(6)
    IF TF>3 THEN TF=TF+3
